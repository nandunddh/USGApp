...
const Login = ({ navigation }) => {
  ...
  //   password: "",
  // });
  const { isLogin, setIsLogin, isAdmin, setIsAdmin } = useContext(MyContext)
  ...
  useEffect(() => {
    ...
    console.log("Data1 = ", isLogin);
  }, [response, isLogin, isAdmin])
  ...
  const handleLogin = () => {
    ...
    else if (email === "nandu@test.com" && password === "test-12345") {
      // alert('Login Success!')
      setIsLogin(true);
    }
    else {
      if (email !== "nandu@test.com" && "nandu@admin.com") {
        alert('Invalid email');
